...
@section('content-header')
    <h1>{{__('inventory.create_inventory_stock')}}</h1>
    <ol class="breadcrumb">
        <li>
    ...
    <div class="box box-default">
        ...
        <form method="POST" action="{{ route('inventory_stocks.store') }}" id="create_inventory_stock_form"
            ...
            <div class="box-body">
                ...
                @include ('inventory_stocks.form_add', ['inventoryStock' => null,])
            </div>

            <div class="box-footer">
                <button type="submit" class="btn btn-primary pull-right">{{__('inventory.add_inventory_stock')}}</button>
            </div>
        </form>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.select-admin-lte').select2().on('change', function () {
                location.href = '{{ url('inventory_stocks/create/inventories') }}' + '/' + $(this).val();
            });
        });
    </script>
@endsection
