    ...
    <div class="col-md-6">
        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
            ...
            <input class="form-control" name="title" type="text" id="title"
                value="{{ old('title', optional($livestocksType)->title) }}" minlength="1" maxlength="255" required>
...
'Inactive' => 'Inactive'] as $key => $text)
                <option value="{{ $key }}" {{ old('status', optional($livestocksType)->status) == $key ? 'selected' : '' }}>
                    {{ $text }}
                </option>
                @endforeach
            </select>
